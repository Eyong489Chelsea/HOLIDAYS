...
export const useHolidayRequestStore = defineStore("holiday", () => {
  ...
  const rejectHolidayRequest = async (id):Promise<RequestResponse<void>> => {
    return handelRequest(async () => {
      await HolidayRequestService.updateHolidayRequest1({id, status: HOLIDAY_STATUS.REJECTED})
      return;
    });
  }

  const approveHolidayRequest = async (id):Promise<RequestResponse<void>> => {
    return handelRequest(async () => {
      await HolidayRequestService.updateHolidayRequest1({id, status: HOLIDAY_STATUS.APPROVED})
      return;
    });
  }
